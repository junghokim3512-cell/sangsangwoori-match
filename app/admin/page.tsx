...
function StatusBadge({ status }: { status: SeniorRow["seniorStatus"] }) {
  if (status === "assigned")
    return (
      <Badge className="text-base px-3 py-1 bg-green-600 hover:bg-green-600">
        배정 완료
      </Badge>
    );
  if (status === "pending")
    return (
      <Badge className="text-base px-3 py-1 bg-yellow-500 hover:bg-yellow-500 text-white">
        매칭 대기
      </Badge>
    );
  return (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-lg font-bold text-center">추천 보기</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.id}>
                  <TableCell className="text-lg font-semibold py-4">{row.name}</TableCell>
                  <TableCell className="text-lg py-4">{row.region}</TableCell>
                  <TableCell className="text-lg py-4">{row.desired_job}</TableCell>
                  ...
                  <TableCell className="text-center py-4">
                    <Link
                      ...
                      className="inline-flex items-center justify-center rounded-lg border-2 border-primary bg-background px-5 py-3 text-base font-bold text-primary hover:bg-primary hover:text-primary-foreground transition-colors min-h-[48px]"
                    >
                      추천 보기
                    </Link>
                  </TableCell>
